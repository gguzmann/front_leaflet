...
export const SettingLayer = () => {
  const { layer, setLayer, color, name } = useSetting()
  ...
  return (
    <div className='p-3 min-h-screen'>
        ...
        <div className='flex flex-wrap gap-5  pb-5'>
          {
            layersTile.map((x, i) => (
              <div key={i} className={`  cursor-pointer hover:outline hover:outline-yellow-500 hover:outline-2 p-1 ${layer === x.url && 'outline outline-yellow-500'}`} onClick={() => setLayer(x.url)}>
                <MapContainer style={{ height: '150px', width: '300px' }} center={[-45.5712, -72.0685]} zoom={10} scrollWheelZoom={false} zoomControl={false} dragging={false}>
                  <TileLayer
      ...
      <button onClick={handleSaveSetting} className={`mt-5 flex gap-3 items-center justify-center ${color || 'bg-sky-700'} hover:bg-opacity-80 text-white font-bold py-2 px-4 rounded w-full`}>
        <div>
          Save Layer
        </div>
        <div>
